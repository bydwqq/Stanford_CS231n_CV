...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(iter_num):
        hidden, cache_tmp = rnn_step_forward(x[:, i, :], hidden, Wx, Wh, b)
        cache.append(cache_tmp)
        h = np.concatenate((h, hidden[:, np.newaxis, :]), axis=1)
    h = h[:, 1:, :]
...
def rnn_backward(dh, cache):
    ...
    for i in range(T-1, -1, -1):
        dxi, dh0, dWxi, dWhi, dbi = rnn_step_backward(dh0 + dh[:, i, :], cache[i])
        dx = np.concatenate((dxi[:, np.newaxis, :], dx), axis=1)
        dWx += dWxi
        dWh += dWhi
        db += dbi
    dx = dx[:, :T, :]
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    dgates = np.concatenate((di, df, do, dg), axis=1)
    dWx = x.T @ dgates
    dWh = prev_h.T @ dgates
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    next_c  = np.zeros((N, H))
    for i in range(T):
        next_h, next_c, cachei = lstm_step_forward(x[:,i,:], next_h, next_c, Wx,
                                                   Wh, b)
        h = np.concatenate((h, next_h[:, np.newaxis, :]), axis=1)
        cache.append(cachei)
    h = h[:,1:,:]
...
def lstm_backward(dh, cache):
    ...
    db = np.zeros(4*H)
    for i in range(T-1, -1, -1):
        dxi, dh0, dc, dWxi, dWhi, dbi = lstm_step_backward(dh0 + dh[:, i, :], dc, cache[i])
        dx = np.concatenate((dxi[:, np.newaxis, :], dx), axis=1)
        dWx += dWxi
        dWh += dWhi
        db += dbi
