...
def discriminator_loss(logits_real, logits_fake):
    ...
    N = logits_real.shape
    label = torch.cat((
        torch.ones(size=N).type(dtype),
        torch.zeros(size=N).type(dtype)
    ), dim=0).squeeze()
    scores=torch.cat((logits_real, logits_fake))
    loss = 2 * bce_loss(scores, label)
...
def ls_generator_loss(scores_fake):
    ...
    label = torch.ones_like(scores_fake).type(dtype)
    loss = torch.mean((scores_fake-label)**2) / 2
