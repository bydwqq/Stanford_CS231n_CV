...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for i in range(2*N):
            if i != k:
                denominator1 += torch.exp(sim(z_k, out[i])/tau)
        for i in range(2*N):
            if i != k+N:
                denominator2 += torch.exp(sim(z_k_N, out[i])/tau)
        l1 = -torch.log(torch.exp(sim(z_k, z_k_N)/tau) / denominator1)
        l2 = -torch.log(torch.exp(sim(z_k_N, z_k)/tau) / denominator2)
        total_loss += l1 + l2
...
def compute_sim_matrix(out):
    ...
    sim_matrix = torch.sum(out1*out2, dim=2) / \
                 torch.linalg.norm(out, dim=1, keepdim=True) / \
                 torch.linalg.norm(out, dim=1, keepdim=True).T
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    numerator = torch.exp(torch.cat([pos_pairs, pos_pairs]) / tau)
